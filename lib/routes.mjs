...
router.get('/files', async (ctx, next) => {
    ...
    content = content.replace('%version%', NODEFILEMANAGER.VERSION);
...
router.get('/api/*event', async (ctx, next) => {
    ...
    setInterval(() => {
        ...
        //dm('sending meta for', Object.keys(currentUploads).length, 'files');
        ...
        if (!type) {
            ...
            await new Promise((resolve, reject) => {
                ...
                ctx.req.on('error', async err => {
                    d('Upload: Error: ' + p);
                    await fs.rename(fTmpUploading, fTmpError).catch(_ => { });
