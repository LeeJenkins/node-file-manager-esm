...
let debugTrace = () => {};
if (DEBUG_TRACE) {
    debugTrace = (...args) => console.log.apply(null, args);
}
...
FMApp.controller("FileManagerCtr", [
    ...
    function ($scope, $http, $location) {
        ...
        $scope.isLoginQuery = false;
        $scope.isAuthNone = false;
        $scope.isAuthRest = false;
        $scope.isAuthBasic = false;
        $scope.showFileViewer = false;
        $scope.showDirectoryListing = true;

        var queryAuthStatus = function () {
            $scope.isLoginQuery = true;
            var conf = {method: "GET", url: "auth/status", timeout: 10 * 1000};
            $http(conf)
                .then(
                    function (response) {
        ...
        var queryAuthType = function () {
            var conf = {method: "GET", url: "auth/type", timeout: 10 * 1000};
            debugTrace(`${conf.method} ${conf.url}`);
            $http(conf)
                .then(
                    function (response) {
                        FM.authType = response.data.authType;
                        if (FM.authType === 'none') {
                            $scope.isAuthNone = true;
                        }
                        if (FM.authType === 'basic') {
                            $scope.isAuthBasic = true;
                            queryAuthStatus();
                        }
                        if (FM.authType === 'rest') {
                            $scope.isAuthRest = true;
                            queryAuthStatus();
                        }
                    },
                    function (response) {
        ...
        var handleHashChange = function (hash) {
            ...
            debugTrace("Hash change: " + hash);
            var relPath = hash.slice(1);
            FM.curHashPath = hash;
            FM.curBreadCrumbPaths = hash2paths(relPath);
            debugTrace("LOG HANDLER for handleHashChange -- relPath =",relPath);
            if (relPath.endsWith('/')) {
                FM.curFolderPath = relPath;
                debugTrace(`handleHashChange calling setCurFiles(FM.curFolderPath)`);
                setCurFiles(relPath);
            }
            else  {
                viewFile({relPath});
        ...
        $scope.$watch(
            ...
            function (val) {
                debugTrace(`$scope.$watch calling handleHashChange()`);
        ...
        $scope.closeFileViewer = function() {
            const folderPath = (FM.curHashPath
                .replace('#','')
                .replace(/\/[^/]+$/,'/')
            );
            ...
            $http(conf)
                .then(
                    function (response) {
                        ...
                        console.log(`FM.toggleShowAllFiles calling handleHashChange()`);
        ...
        FM.watchFileEvents = function () {
            ...
            source.addEventListener('message', function (e) {
                debugTrace("BEGIN HANDLER for EventSource('/api/*event') message event");
                ...
                for (var key3 in files) {
                    if (fNames.indexOf(key3) === -1) {
                        debugTrace(`HANDLER for EventSource('/api/*event') message event calling setCurFiles(FM.curFolderPath)`);
        ...
        var httpRequest = function (method, url, params, data, config) {
            ...
            const skipHashNotify = config && config.skipHashNotify;
            for (var k in config) {
                if (config.hasOwnProperty(k)) {
                    conf[k] = config[k];
                }
            }
            debugTrace(`httpRequest: ${method} ${url}`);
            return new Promise((resolve, reject) => {
                $http(conf)
                .then(response => {
                    // FM.successData = response.data;
                    if (!skipHashNotify) {
                        debugTrace(`httpRequest() calling handleHashChange()`);
                        handleHashChange(FM.curHashPath);
                    }
                    response.url = url;
                ...
                .catch(reason => {
                    if (reason && 'status' in reason) {
                        if (reason.status !== 0) {
                            FM.errorData = " " + reason.status + ": " + reason.data;
                        }
                        else {
                            console.error('Ajax may not get info on errors. HTTPS may help.');
                        }
        ...
        var viewFile = (file) => {
            ...
            $scope.showFileViewer = true;
            $scope.showDirectoryListing = false;

            // Fetch file content
            debugTrace(`HANDLER for viewFile -- file object: ${JSON.stringify(file, null, 4)}`);
            $scope.fileType = '';
            $scope.fileData = '';
            ...
            httpRequest("GET", url, null, null, {
                ...
                // Update scope variables for binding
                $scope.$apply(() => {
                    $scope.fileUrl = fileUrl;
                    $scope.fileType = fileType;
                    $scope.fileName = file.name;
                    $scope.fileData = response.data;
                    debugTrace(`HANDLER for viewFile -- $scope.fileUrl: ${$scope.fileUrl}`);
                });
            });
        }

        FM.clickFile = function(file) {
            const FILE_ACTION = 'view'; // TBD: let user configure/select this option
            if (file.folder) {
                // open folder by setting url hash
                $location.path(decodeURIComponent(file.relPath));
            } else if (FILE_ACTION === 'dowload') {
                // download file
                downloadFile(file);
            } else if (FILE_ACTION === 'view') {
                $location.path(decodeURIComponent(file.relPath));
            }
        };
        ...
        FM.upload = function () {
            ...
            for (uploadFile of FM.uploadFile) {
                ...
                httpRequest("POST", url, { type: "UPLOAD_FILE" }, uploadFile, {
        ...
        function basicAuthLogout(){
            ...
            const logoutUrl = `${protocol}${new Date().getTime()}@${window.location.host}/auth/logout`;
            $http.get(logoutUrl).then(refresh, refresh);
            function refresh() {
                $scope.isLoggedIn = false;
            }
        }
        ...
        FM.logout = function() {
            if (FM.authType === AUTH_TYPE_BASIC) {
                basicAuthLogout();
            }
            if (FM.authType === AUTH_TYPE_REST) {
                httpRequest("POST", "/logout", null, null);

                $http.post('/logout').then(function(response) {
                    $location.path('/'); // Redirect to the main page after logout
                }, function(error) {
